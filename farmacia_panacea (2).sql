...
--
-- Servidor: 127.0.0.1
-- Tiempo de generación: 16-06-2025 a las 06:30:00
...
(1, 'sebastian', 'avilez', 'Kra 21 #09-34', '45302484', 'fkfewfegf@gmail.com');
...
(2, 1, 'Pendiente', 50000.00, NULL, NULL, 10, 0.00),
...
(5, '12', '2025-06-03', '2027-02-23', 2, 'Noxpirin', 90000.00, 87);
...
(2, 'Dolorsinfen', 'Pastilla para el dolor abdominal', 'Tableta de Pastilla', 4500.00, 40, '2025-06-14'),
(3, 'Paracetamol', 'pastillas para el dolor de cabeza', 'Tableta de pastilla', 3500.00, 22, '2025-06-14'),
...
--
DELIMITER $$
CREATE TRIGGER `tr_insertar_producto` AFTER INSERT ON `productos` FOR EACH ROW BEGIN
  DECLARE lote_total INT;

  SELECT total INTO lote_total
...
CREATE ALGORITHM=UNDEFINED DEFINER=`root`@`localhost` SQL SECURITY DEFINER VIEW `vw_proximos_a_vencer`  AS SELECT `p`.`nombre` AS `nombre`, `l`.`numero_lote` AS `numero_lote`, `l`.`fecha_vencimiento` AS `fecha_vencimiento` FROM (`productos` `p` join `lotes_productos` `l` on(`p`.`id_producto` = `l`.`id_producto`)) WHERE `l`.`fecha_vencimiento` <= curdate() + interval 30 day ;
...
--
ALTER TABLE `creditos_clientes`
  MODIFY `id_credito_cliente` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
...
--
ALTER TABLE `pagos_creditos_clientes`
  MODIFY `id_pago_credito` int(11) NOT NULL AUTO_INCREMENT, AUTO_INCREMENT=5;
